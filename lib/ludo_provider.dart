...
class LudoProvider extends ChangeNotifier {
  ...
  bool checkToKill(
    ...
    // Check each player that exists in the current game
    for (var player in players) {
      if (player.type == type) continue; // Skip the current player
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (pawn.step > -1 &&
            !LudoPath.safeArea
                .map((e) => e.toString())
                .contains(player.path[pawn.step].toString())) {
  ...
  void validateWin(LudoPlayerType color) {
    ...
    // Game ends when all players except one have won
    if (winners.length == _playerCount - 1) {
      _gameState = LudoGameState.finish;
    }
  }

  void startGame({int playerCount = 4}) {
    _playerCount = playerCount;
    winners.clear();
    players.clear();

    // Always add green and yellow players
    players.add(LudoPlayer(LudoPlayerType.green));
    players.add(LudoPlayer(LudoPlayerType.yellow));

    // Add blue player if 3 or more players
    if (playerCount >= 3) {
      players.add(LudoPlayer(LudoPlayerType.blue));
    }

    // Add red player if 4 players
    if (playerCount >= 4) {
